...
using ChannelKeypoints = std::vector<std::vector<Keypoint>>;

class KeypointDetectionModel
{
public:
    KeypointDetectionModel(std::string model_path);

    void computeKeypoints(const cv::Mat &image, std::vector<std::pair<SoccerPitch3D::PointID, std::vector<Point2D>>> &outPointDict);
...
private:
    ChannelKeypoints getKeypointsFromHeatmapBatchMaxpool(const torch::Tensor &heatmap,
                                                         ...
                                                         bool returnScores);
    void retrieveSemanticPoints(ChannelKeypoints detectedPoints,
